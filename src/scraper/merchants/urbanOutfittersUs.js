...
export default class UrbanOutfittersUs extends Merchant {
  constructor(url) {
    super(url);
  }
  ...
  scrapeMulti(element) {
    ...
    try {
      // Product Name
      let productName = element.querySelector('.c-product-tile__h3 > span');
      if (productName) {
      ...
      // Image
      let imageUrlContainer = element.querySelector('img.c-product-tile__img');
      ...
      // Prices
      let price = element.querySelector('.c-product-meta__original-price');
      let currentPrice = element.querySelector('.c-product-meta__current-price');
      ...
      // Url
      let url = element.querySelector('a.c-product-tile__image-link');
      if (url) {
        result['url'] = url.href;
      }
