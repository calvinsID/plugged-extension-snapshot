...
export default class GuessFactoryCa extends Merchant {
  constructor(url) {
    super(url);
  }
      ...
      if (priceContainer && priceContainer.length > 0) {
        ...
        if (currentPrice && currentPrice.length > 0) {
          ...
          if (retailPrice && retailPrice.length > 0) {
            ...
            if (retailPrice && retailPrice.length > 0) {
              result['retailPrice'] = parseFloat(retailPrice[0].innerText.trim().substring(0, retailPrice[0].innerText.trim().length - 4).replace(',', ''));
            }
          }
          currentPrice = currentPrice.find('span.priceVal');
          if (currentPrice && currentPrice.length > 0) {
            result['currentPrice'] = parseFloat(currentPrice[0].innerText.trim().split('CAD')[0].trim().replace(',', ''));
          }
          ...
          if (currentPrice2 && currentPrice2.length > 0) {
            result['currentPrice'] = parseFloat(currentPrice2[0].innerText.trim().split('CAD')[0].trim().replace(',', ''));
          }
