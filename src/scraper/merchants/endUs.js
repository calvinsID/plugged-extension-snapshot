...
export default class EndUs extends Merchant {
  constructor(url) {
    super(url);
  }
    ...
    try {
      ...
      if (price && price.length > 0) {
        ...
        if (comparePrice.length > 0 && currentPrice.length > 0) {
          if (parseFloat(comparePrice[0].innerHTML.substring(1).replace(',', '')) !== parseFloat(currentPrice[0].innerHTML.substring(1).replace(',', ''))) {
            // On sale
            result['retailPrice'] = parseFloat(comparePrice[0].innerHTML.substring(1).replace(',', ''));
            result['currentPrice'] = parseFloat(currentPrice[0].innerHTML.substring(1).replace(',', ''));
          } else {
            // Not on sale
            result['retailPrice'] = parseFloat(currentPrice[0].innerHTML.substring(1).replace(',', ''));
