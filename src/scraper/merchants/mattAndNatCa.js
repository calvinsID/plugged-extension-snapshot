...
export default class MattAndNatCa extends Merchant {
  constructor(url) {
    super(url);
  }
  ...
  scrape() {
    let result = {
      ...
      imageUrl: null,
      sizes: [],
      merchant: 'Matt & Nat (CA)',
      scrapedWithoutErrors: true
    }

    try {
      // Product Name
      let productName = $('h1.page-title');
      ...
      // Image
      let imageUrl = $('a.product_img > img');
      ...
      // Prices
      let priceContainer = $('.product-info-main');
  ...
  scrapeMulti(element) {
    ...
    try {
      // Product Name
      let productName = element.querySelector('.product-item-link');
      if (productName) {
